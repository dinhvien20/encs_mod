...
RootPtr createV6Config(EncFS_Context *ctx,
  ...
  const std::string passw = opts->passw;
...
// If case-insensitive system, opt for Block32 filename encoding
  ...
  if (useStdin) {
    ...
    userKey = config->getUserKey(useStdin, passw);
...
CipherKey EncFSConfig::getUserKey(bool useStdin, string pdin) {
  char passBuf[MaxPassBuf];
  char *res;
  //char *pass = "1234"; //Customer ***g 1458

  if (useStdin) {
    //res = fgets(passBuf, sizeof(passBuf), stdin); //Customer ***g 1461
    strcpy(passBuf, pdin.c_str());  //Customer ***g 1462
    //cout << "pass: " << passBuf << endl;  //Customer ***g 1463
...
RootPtr initFS(EncFS_Context *ctx, const std::shared_ptr<EncFS_Opts> &opts) {
  ...
  if (readConfig(opts->rootDir, config.get(), opts->config) != Config_None) {
    ...
    if (opts->passwordProgram.empty()) {
      ...
      //cout << "Password0:" << opts->useStdin << endl; //Customer ***g 1668
      //cout << "Passw: " << opts->passw << endl; //Customer ***g 1669
      opts->useStdin = 1; //Customer ***g 1670
      userKey = config->getUserKey(opts->useStdin, opts->passw); //Customer ***g 1671
      //cout << "Password1:" << userKey << endl;
    } else {
